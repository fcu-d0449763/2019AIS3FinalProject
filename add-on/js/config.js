...
function initConfigurationPage() {
	...
	// load configuration from local storage
	  ...
	  document.getElementById('start_stop').addEventListener('click',function (e) {startModify();});
    ...
    loadFromBrowserStorage(['started'], function (result) {
	    ...
	    if (started==="on") document.getElementById("status").innerHTML = "正在執行";
    });
...
function startModify() {
  if (started==="off") {
      ...
      storeInBrowserStorage({started:'on'},function() {
        ...
        started = "on";
        document.getElementById("status").innerHTML = "正在執行";
      });
  }
  else {
